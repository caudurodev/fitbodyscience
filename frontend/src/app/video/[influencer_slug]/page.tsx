...
export default function Page({ params }: { params: { influencer_slug: string } }) {
  const router = useRouter()
  const { data, loading } = useQuery(
    GET_INFLUENCER_CONTENT_QUERY,
    { variables: { influencerSlug: params?.influencer_slug, mediaType: 'youtube_video' }, fetchPolicy: 'network-only' }
  )
