...
import { Link, Card, CardBody, Button, CardHeader } from "@nextui-org/react";
    ...
    return (<li className="mb-8 scroll-mt-28" id={`assertion_${assertionIndex}`} >
        <Card shadow="none" radius="sm" className={isActive ? 'border-primary border-4' : ''}>
            <CardHeader className="flex-col items-start gap-2">
                <Link href={`/assertions/${assertions_content?.assertion?.slug}`} ><h4 className="text-xl font-bold text-primary-500">{assertions_content.assertion.text}</h4></Link>
